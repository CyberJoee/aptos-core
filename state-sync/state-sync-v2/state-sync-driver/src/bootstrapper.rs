    ...
    async fn verify_account_states_indices(
        ...
        if expected_start_index != account_state_with_proof.first_index {
            ...
            return Err(Error::VerificationError(format!(
                ...
                expected_start_index, account_state_with_proof.first_index
            )));
        }

        // Verify the number of account blobs is valid
        let expected_num_accounts = account_state_with_proof
            .last_index
            .checked_sub(account_state_with_proof.first_index)
        ...
        let num_accounts = account_state_with_proof.raw_values.len() as u64;
        ...
        let expected_end_index = account_state_with_proof
            .first_index
        ...
        if expected_end_index != account_state_with_proof.last_index {
            self.terminate_active_stream(notification_id, NotificationFeedback::InvalidPayloadData)
                .await?;
            return Err(Error::VerificationError(format!(
                "The expected end index was invalid! Expected: {:?}, received: {:?}",
                expected_num_accounts, account_state_with_proof.last_index,
            )));
        }
    ...
    async fn process_account_states_payload(
            ...
            .initialized_state_snapshot_receiver
        {
            let version = self
                ...
                .ledger_info()
                .version();
            let expected_root_hash = account_state_with_proof.root_hash;
        ...
        self.verify_account_states_indices(notification_id, &account_state_with_proof)
            .await?;

        // TODO(joshlind): Verify the sparse merkle tree proof is valid!

        // Process the account states chunk and proof
        let last_account_index = account_state_with_proof.last_index;
        if let Err(error) = self
            .storage_synchronizer
            .save_account_states(notification_id, account_state_with_proof)
        {
            self.terminate_active_stream(notification_id, NotificationFeedback::InvalidPayloadData)
                .await?;
